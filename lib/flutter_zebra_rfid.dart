...
class FlutterZebraRfid {
  static const MethodChannel _channel = const MethodChannel('flutter_zebra_rfid');
    ...
    _channel.setMethodCallHandler((call) async {
      ...
      switch (call.method) {
        case "eventReaderAppeared":
          assert(args["name"] != null && args["name"] is String,
              "name argument is not defined or is not a String");
          assert(args["id"] != null && args["id"] is String,
              "id argument is not defined or is not a String");
        ...
        case "eventReadNotify":
          assert(args["reader"] != null && args["reader"] is Map,
              "reader argument is not defined or is not a Map");
          assert(args["tagData"] != null && args["tagData"] is Map,
              "tagData argument is not defined or is not a Map");
          if (args["tagData"]["tagId"] != null) {
            _getAvailableReaderInfo(args["reader"])
                ._eventReadNotify(TagData.fromMap(args["tagData"]));
          }
          break;

        case "eventStatusNotify":
          assert(args["reader"] != null && args["reader"] is Map,
              "id argument is not defined or is not a Map");
          assert(args["event"] != null && args["event"] is num,
              "event argument is not defined or is not a num");
          ...
          _getAvailableReaderInfo(args["reader"])._eventStatusNotify(event, data: data);
          break;

        case "eventProximityNotify":
          assert(args["reader"] != null && args["reader"] is Map,
              "id argument is not defined or is not a Map");
          assert(args["proximityPercent"] != null && args["proximityPercent"] is num,
              "proximityPercent argument is not defined or is not a num");
          _getAvailableReaderInfo(args["reader"])
              ._eventProximityNotify(args["proximityPercent"]);
          break;
        ...
        case "eventTriggerNotify":
          ...
          assert(args["event"] != null && args["event"] is num,
              "event argument is not defined or is not a num");
          _getAvailableReaderInfo(args["reader"])._eventTriggerNotify(
              args["event"] == 1 ? TriggerEvent.released : TriggerEvent.pressed);
          break;

        case "eventBatteryNotify":
          assert(args["reader"] != null && args["reader"] is Map,
              "id argument is not defined or is not a Map");
          assert(args["cause"] != null && args["cause"] is String,
              "cause argument is not defined or is not a String");
          assert(args["isCharging"] != null && args["isCharging"] is bool,
              "isCharging argument is not defined or is not a bool");
          assert(args["powerLevel"] != null && args["powerLevel"] is int,
              "powerLevel argument is not defined or is not a int");
          _getAvailableReaderInfo(args["reader"])
              ._eventBatteryNotify(args["cause"], args["powerLevel"], args["isCharging"]);
          break;
